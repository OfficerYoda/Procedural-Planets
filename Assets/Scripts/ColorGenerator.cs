...
public class ColorGenerator {
    ...
    public void UpdateSettings(ColorSettings settings, int seed) {
        this.settings = settings;
        if(texture == null || texture.height != settings.biomeColorSettings.biomes.Length)
            texture = new Texture2D(textureResolution * 2, settings.biomeColorSettings.biomes.Length, TextureFormat.RGBA64, false);
        biomeNoiseFilter = NoiseFilterFactory.CreateNoiseFilter(settings.biomeColorSettings.noise, seed);
    }
    ...
    public float BiomePercentFromPoint(Vector3 pointOnUnitySphere) {
        float heightPercent = (pointOnUnitySphere.y + 1) / 2f;
        heightPercent += (biomeNoiseFilter.Evaluate(pointOnUnitySphere) - settings.biomeColorSettings.noiseOffset) * settings.biomeColorSettings.noiseStrength;
        float biomeIndex = 0;
